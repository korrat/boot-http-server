...
const getChirpsBy = `-- name: GetChirpsBy :many
SELECT
    id, created_at, updated_at, body, user_id
FROM
    chirps
...
func (q *Queries) GetChirpsBy(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChirpsBy, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
